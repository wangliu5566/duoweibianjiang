<template>
  <div class="eventsList" :style="{height:winHeight+'px'}" @click="conMenuShow=false" @contextmenu.prevent="openContextMenu($event)">
    <!-- 搜索 -->
    ...
    <table cellspacing="0" style="width: 100%;">
      <thead>
        <tr>
          ...
          <th class="line-2" :class="showDynasty?'line2BG line-2':'line-2'" @click="ascendingOrder()">
             <span style="display: flex;justify-content: space-between;align-items: center;">
               <span style="display: flex;justify-content: flex-start;align-items: center;">
                 时期
                 <i class="order" v-if="!isUP" @click.stop="ascendingOrder()"></i>
                 <i class="order1" v-if="isUP" @click.stop="ascendingOrder()"></i>
               </span>
      ...
      <tbody>
        <tr v-if="tableData.length!=0" v-for="item in tableData" :thisId="item.Id" :thisTitle="item.Title" @click="goEventDetail(item.Id,item.Title)">
        ...
        <tr v-if="tableData.length==0">
          <td colspan="3" style="text-align: center;line-height: 40px;">暂无数据</td>
        </tr>
      </tbody>
    </table>
    <!-- 分页 -->
...
export default {
  data() {
    return {
      ...
      preCurPage:1,
      totalCount: 0,
      type: 2, //1-人物2-事件3-地区4-朝代5-方位6-分类属性
      dynastyArr: [],
      checkedNum: 0,
      loading: false,
      conMenuShow: false,
  ...
  methods: {
    ...
    handleIconClick() {
      ...
      // this.pageCount = 15,
       ...
       if(this.eventName == '') {
        ...
        this.showSearchKeyword = this.eventName;
        this.showEventList();
    },
    //重置搜索框
    resetSearchKey() {
          ...
          if (winResult.Success) {
            switch (this.type) {
              ...
              case 4:
                ...
                //找出已筛选项
                arr.forEach((item,index)=>{
                   if(this.checkedCities.indexOf(item.LocalId) !== -1) {
                      item.isChecked = true;
                   }
                })
    ...
    //跳转到事件详情
    goEventDetail(id, title) {
      if (env == "dev") {
        this.$router.push({ path: '/eventDetail', query: { eventId: id } })
      } else if (env == "prod") {
    ...
    rightDetail() {
      if (this.conMenuShow) {
        ...
        this.goEventDetail(thisId,thisTitle)
      }
    },
  }
}
